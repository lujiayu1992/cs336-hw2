...
def run_naive_ddp(
  ...
  local_input_ids = rearrange(
      global_input_ids, "(w b) ... -> w b ...", w=world_size
  )[rank].to(device)
  local_target_ids = rearrange(
      global_target_ids, "(w b) ... -> w b ...", w=world_size
  )[rank].to(device)
  ...
  for step in range(5):
    ...
    loss = F.cross_entropy(
        logits.view(-1, XL_CONFIG["vocab_size"]), local_target_ids.view(-1)
    )
